...
public class Validator {
  ...
  protected final String errorMessage_checkLengthFixed = "{0}の長さが不正です。\n固定長({1})";
  ...
  protected final String errorMessage_checkDateTime_compare =
      "{0}が{1}よりも未来の日時になっています。{0}({2}),{1}({3})";
  protected final String errorMessage_checkBase64 = "画像形式が不正です。";
  protected final String errorMessage_checkInactiveTime =
      "使用不可開始/終了日時を入力する場合は、動作状況に3:使用不可、または4:メンテナンス中を入力してください。";
  ...
  protected final String errorMessage_checkBase64_MIME = "{0}はサポート対象外のファイル形式です。";
  protected final String errorMessage_checkDateTime_equal =
      "{0}と{1}が同一の日時になっています。{0}({2}),{1}({3})";

  protected List<String> details = new ArrayList<>();

  /**
   * バリデーションエラーがある場合はValidationErrorExceptionをスローする
   *
   ...
   * @param str チェック文字
   * @param name チェック対象の名称
   * @param fixedLength チェック対象の固定長
   */
  protected void checkLengthFixed(String name, String str, int fixedLength) {
    if (StringUtils.hasText(str) && str.length() != fixedLength) {
      details.add(MessageFormat.format(errorMessage_checkLengthFixed, name, fixedLength));
    }
  }
  ...
  protected void checkRange(String name, Integer value, Integer[] range) {
    if (value != null && Arrays.binarySearch(range, value) < 0) {
      this.details.add(
          MessageFormat.format(errorMessage_checkRange2, name, Arrays.toString(range)));
    }
  }

  /**
   * 入力数値範囲チェック(BigInteger)
   *
   * <p>BigIntegerがInteger範囲外の場合は範囲エラー、範囲内の場合はInteger配列でチェック
   *
   * @param name チェック対象名称
   * @param value チェック対象
   * @param range 範囲
   */
  protected void checkRange(String name, BigInteger value, Integer[] range) {
    if (value != null) {
      // Integer範囲チェック（オーバーフロー対策）
      BigInteger minInt = BigInteger.valueOf(Integer.MIN_VALUE);
      BigInteger maxInt = BigInteger.valueOf(Integer.MAX_VALUE);
      if (value.compareTo(minInt) < 0 || value.compareTo(maxInt) > 0) {
        // Integer範囲外の場合、配列形式でエラーを出す
        this.details.add(
            MessageFormat.format(errorMessage_checkRange2, name, Arrays.toString(range)));
        return;
      }
      // Integer範囲内なら変換してチェック
      checkRange(name, value.intValue(), range);
    }
  }
   ...
   * @param max 最大値
   */
  protected void checkRange(String name, BigInteger value, BigInteger min, BigInteger max) {
    if (value != null
        && (min.compareTo(max) > 0 || min.compareTo(value) > 0 || value.compareTo(max) > 0)) {
      this.details.add(MessageFormat.format(errorMessage_checkRange, name, min, max));
    }
  }
  ...
  protected void checkValue(String name, String value, Pattern pattern) {
    if (StringUtils.hasText(value) && pattern != null && !pattern.matcher(value).matches()) {
        ...
        if (lowerDt.compareTo(upperDt) > 0) {
          this.details.add(
              MessageFormat.format(
                  errorMessage_checkDateTime_compare, lowerName, upperName, lower, upper));
        }
      } catch (DateTimeParseException e) {
   ...
   * @param upper 終了日時(文字列)
   */
  protected void checkNotEqualDateTime(
      String lowerName, String upperName, String lower, String upper) {
    if (StringUtils.hasText(lower) && StringUtils.hasText(upper)) {
      try {
        ZonedDateTime lowerDt = ZonedDateTime.parse(lower);
        ZonedDateTime upperDt = ZonedDateTime.parse(upper);
        if (lowerDt.compareTo(upperDt) == 0) {
          this.details.add(
              MessageFormat.format(
                  errorMessage_checkDateTime_equal, lowerName, upperName, lower, upper));
        }
      } catch (DateTimeParseException e) {
        ; // 日時文字列が不正な場合は何もしない。(書式チェックは別に行う)
      }
