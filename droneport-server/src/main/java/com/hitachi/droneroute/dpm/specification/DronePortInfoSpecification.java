...
public class DronePortInfoSpecification<T> {
  ...
  public Specification<T> dronePortIdEqual(String dronePortId) {
    return !StringUtils.hasText(dronePortId)
        ? null
        : (root, query, builder) -> {
  ...
  public Specification<T> serialNumberContains(String serialNumber) {
    return !StringUtils.hasText(serialNumber)
        ? null
        : (root, query, builder) -> {
   ...
   * ポート形状
   *
   * @param portTypes ポート形状
   * @return Specification
   */
  public Specification<T> portTypeContains(Integer[] portTypes) {
    return portTypes == null || portTypes.length == 0
        ? null
        : (root, query, builder) -> {
          return builder.or(
              Stream.of(portTypes)
  ...
  public Specification<T> supportDroneTypeContains(String supportDroneType) {
    return !StringUtils.hasText(supportDroneType)
        ? null
        : (root, query, builder) -> {
  ...
  public Specification<T> activeStatusContains(Integer[] activeStatus) {
    return activeStatus == null || activeStatus.length == 0
        ? null
        : (root, query, builder) -> {
          return builder.or(
  ...
  private Predicate activeStatusContains(
      Root<T> root, CriteriaBuilder builder, List<Integer> activeStatus) {
    return builder.or(
        activeStatus.stream()
            .map(e -> builder.equal(root.get("dronePortStatusEntity").get("activeStatus"), e))
            .collect(Collectors.toList())
  ...
  public Specification<T> activeStatusInner2(List<Integer> statuses, Timestamp current) {
    return (root, query, builder) ->
        builder.and(
            activeStatusContains(root, builder, statuses),
            builder.equal(
                root.get("dronePortStatusEntity").get("inactiveStatus"),
                DronePortConstants.ACTIVE_STATUS_UNAVAILABLE),
            builder.greaterThan(
                builder.function(
                    "tsrange_lower",
                    Timestamp.class,
   ...
   * 使用不可日時範囲: {@code lower > 検索実行日時 or upper < 検索実行日時}
   *
   * @param statuses 動作状況
   * @param current 指定日時
   * @return Specification　検索条件
  ...
  public Specification<T> activeStatusInner3(List<Integer> statuses, Timestamp current) {
    return (root, query, builder) ->
        builder.and(
            ...
            builder.equal(
                root.get("dronePortStatusEntity").get("inactiveStatus"),
                DronePortConstants.ACTIVE_STATUS_MAINTENANCE),
            builder.or(
                builder.greaterThan(
                    builder.function(
                        "tsrange_lower",
   ...
   * 動作状況(使用不可): 4:メンテナンス中<br>
   * 使用不可日時範囲: {@code lower <= 検索実行日時 かつ upper > 検索実行日時}
   *
   * @param current 指定日時
   * @return 検索条件
   */
